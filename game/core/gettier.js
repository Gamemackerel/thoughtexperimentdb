...
export function beliefCard(belief, { reason = 'You had good reason to' } = {}) {
  ...
  el.innerHTML = `<div class="said">You believed:</div><div class="what">“${esc(belief)}”</div>
    <ul><li>You believed it</li><li>It was true</li><li>${esc(reason)}</li></ul><div class="knew">Did you know it?</div>`;
  document.getElementById('ui').appendChild(el);
  const items = el.querySelectorAll('li');
  return {
    show() { el.classList.add('on'); },
    tick(i, ok = true) { items[i]?.classList.add(ok ? 'yes' : 'no'); },
    ask(text) { if (text) el.querySelector('.knew').textContent = text; el.classList.add('asked'); },
