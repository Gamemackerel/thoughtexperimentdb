...
// nine parts (six planks, the deck, the mast and the sail), two a trip: five trips
...
for (let k = 0; k < 5; k++) {
  ...
  if (k === 0 || k === 3) await shot(`02-carry${k + 1}`);
  await walkTo(16, -0.6); await sleep(3); await press(); await sleep(2.4);
  await sleep(2.4); if (k === 0 || k === 4) await shot(`03-swap${k + 1}`);
}
await sleep(9); await shot('04-rebuild'); await sleep(9); await shot('05-choose');
if (process.argv[2] === 'old') { await walkTo(17, 1.1); } else { await walkTo(17, -1.1); }
