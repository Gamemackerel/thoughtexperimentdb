#!/usr/bin/env bash
# One-time setup: Node deps, Python env for Kokoro TTS + sound design + narration audit, and the Kokoro model files.
# Requires: node 20+, python 3.10+, ffmpeg (brew install ffmpeg), Google Chrome or Puppeteer's bundled Chromium.
set -euo pipefail
cd "$(dirname "$0")/.."
npm install
...
echo "Ready. Try: node engine/render.mjs trolley-problem --preview"
